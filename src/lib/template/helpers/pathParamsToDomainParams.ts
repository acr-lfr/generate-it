...
function addType(
  ...
  if (
    !forceType &&
    requestType &&
    pathObject['x-request-definitions'] &&
    pathObject['x-request-definitions'][requestType]
  ) {
    if (requestType === 'body') {
...
export default function (
  ...
  withPrefix?: string,
  pathNameChange: string = 'pathParams'
) {
  if (!pathObject) {
    return '';
  }
  ...
  const helpers =
    this.ctx && this.ctx.config.data.nodegenRc.helpers ? this.ctx.config.data.nodegenRc.helpers : undefined;
