...
versionCheck(
  ...
  .then(() => {
    console.log(
      `Provided cli args look ok, preceding to build the http layer and any stub files... ${LINEBREAK}`.yellow
    );
    const config: Config = {
    ...
    const call = () => {
      ...
      generateIt(config)
        .then(() => {
          ...
          console.log(
            `${LINEBREAK}Your API files have been output here: `.yellow + cli.program.output.magenta + `.`.green.bold
          );
        })
        .catch(async (err: any) => {
    ...
    const question =
      `Continuing will replace the entire http layer:`.green +
      `
