...
var suggestVersionUpgrade_1 = require("../suggestVersionUpgrade");
// https://semver.org/
describe('suggestVersionUpgrade', function () {
    var jsonDiff;
    it('should suggest upgrades', function () {
        ...
        expect(suggestVersionUpgrade_1.suggestVersionUpgrade(jsonDiff, 'npm rm -rf')).toBe('npm rm -rf @test/missing@1.19.0');
        jsonDiff = { '@test/samever2': { 'Changed To': '^3.1.4-beta', 'from': '1.0.89' } };
        expect(suggestVersionUpgrade_1.suggestVersionUpgrade(jsonDiff, 'npm rm -rf')).toBe('npm rm -rf @test/samever2@3.1.4-beta');
        jsonDiff = { '@types/yamljs': { 'Changed To': '^0.2.31', 'from': '^0.2.30' } };
        expect(suggestVersionUpgrade_1.suggestVersionUpgrade(jsonDiff, 'npm rm -rf')).toBe('npm rm -rf @types/yamljs@0.2.31');
        jsonDiff = { typescript: { 'Changed To': '^3.9.2', 'from': '^3.8.3' } };
        expect(suggestVersionUpgrade_1.suggestVersionUpgrade(jsonDiff, 'npm rm -rf')).toBe('npm rm -rf typescript@3.9.2');
    });
    it('should not suggest downgrades or equivalent versions', function () {
        jsonDiff = { '@test/equal': { 'Changed To': '^5.0.0', 'from': '^5.0.0' } };
        expect(suggestVersionUpgrade_1.suggestVersionUpgrade(jsonDiff, 'npm rm -rf')).toBeUndefined();
    ...
    it('should ignore gibberish', function () {
    ...
    it('can report all changes', function () { return tslib_1.__awaiter(void 0, void 0, void 0, function () {
