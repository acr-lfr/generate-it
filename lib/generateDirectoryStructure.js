...
module.exports = config => new Promise((resolve, reject) => {
  ...
  if (config.mockServer) {
    // delete the mock server content and rebuild from scratch
    fs.removeSync(path.join(target_dir, 'src'))
    fs.removeSync(path.join(target_dir, 'dredd'))
    console.log('./src and ./dredd directories are removed in prep for the mocking server.')
  }
