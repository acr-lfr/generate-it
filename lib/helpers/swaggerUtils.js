const self = {
  ...
  pathParamsToJoi: (param, options = {}) => {
    ...
    if (['string', 'number', 'integer', 'boolean'].includes(type)) {
      ...
      if (param.minLength) validationText += (param.minLength ? `.min(${+param.minLength})` : '')
      if (param.minimum) validationText += (param.minimum ? `.min(${+param.minimum})` : '')
      if (param.maxLength) validationText += (param.maxLength ? `.max(${+param.maxLength})` : '')
      if (param.maximum) validationText += (param.maximum ? `.max(${+param.maximum})` : '')
      if (type === 'string' && param.pattern) validationText += (param.pattern ? `.pattern(new RegExp('${param.pattern}'))` : '')
      ...
      if (param.minItems) validationText += (param.minItems ? `.min(${+param.minItems})` : '')
      if (param.maxItems) validationText += (param.maxItems ? `.max(${+param.maxItems})` : '')
