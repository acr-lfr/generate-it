...
class OpenAPIInjectInterfaceNaming {
  ...
  injectFromSwaggerpaths (path, method) {
    let requestParams = {
      query: {
        name: generateOperationId(_.upperFirst(method) + 'Query', path),
        params: []
      },
      headers: {
        name: generateOperationId(_.upperFirst(method) + 'Headers', path),
        params: []
      },
      body: {
        name: generateOperationId(_.upperFirst(method) + 'Body', path),
        params: []
      },
      path: {
        name: generateOperationId(_.upperFirst(method) + 'Path', path),
        params: []
      },
    }
  ...
  mergeSwaggerInjectedParameters (path, method) {
    Object.keys(this.apiObject.paths[path][method]['x-request-definitions']).forEach((requestType) => {
      ...
      if (!clear) {
        this.apiObject.paths[path][method]['x-request-definitions'][requestType].interface = requestObject
      } else {
  ...
  injectFromSwaggerResponse (path, method) {
    ...
    if (pathResponses && pathResponses['200'] && pathResponses['200'].schema && pathResponses['200'].schema['$ref']) {
      try {
        response['200'] = this.convertRefToOjectPath(pathResponses['200'].schema['$ref'])
      } catch (e) {
        console.error(e)
      }
    }
