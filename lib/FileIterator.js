...
class FileWalker {
  ...
  async fileIteration (root, stats, next) {
    ...
    global.verboseLogging('Dir:' + root)
    global.verboseLogging('File:' + stats.name)
    const targetDir = config.targetDir
    const templatesDir = config.templates
    ...
    if (FileTypeCheck.isInterfaceFile(generationDataObject.file_name)) {
      global.verboseLogging('Interface file: ' + generationDataObject.file_name )
      ...
      global.verboseLogging('Mock|Stub|Operation file: ' + generationDataObject.file_name )
      ...
      global.verboseLogging('General tpl file: ' + generationDataObject.file_name )
